...
except Exception:  # numba/DLL policy, missing package, …
    rembg_remove = None
...
def _fallback_cutout(src):
    """When rembg is unavailable: flood-fill light corner colour → transparent.

    Good enough for plain / cartoon backgrounds (GitHub avatars, studio
    headshots on a wall). Real photos with messy backgrounds still want rembg.
    """
    arr = np.array(src.convert("RGBA"))
    h, w = arr.shape[:2]
    # sample four corners — typical matte colour for avatars
    samples = [arr[2, 2, :3], arr[2, w - 3, :3],
               arr[h - 3, 2, :3], arr[h - 3, w - 3, :3]]
    bg = np.median(samples, axis=0).astype(np.float32)
    rgb = arr[:, :, :3].astype(np.float32)
    dist = np.linalg.norm(rgb - bg, axis=2)
    # soft threshold: near-background → transparent
    alpha = np.clip((dist - 18.0) / 22.0, 0.0, 1.0)
...
def prep(path, crop=None, use_rembg=True):
    ...
    if use_rembg and rembg_remove is not None:
        cut = rembg_remove(src)
    else:
        if use_rembg and rembg_remove is None:
            print("rembg unavailable — using colour-distance cutout",
                  file=sys.stderr)
        cut = _fallback_cutout(src)
...
def main():
    ...
    ap.add_argument("--no-rembg", action="store_true",
                    help="skip rembg; use colour-distance cutout instead")
    args = ap.parse_args()
    ...
    lines = to_lines(prep(args.photo, crop, use_rembg=not args.no_rembg),
                     cols=args.cols)
    if args.preview:
        print("\n".join(lines))
